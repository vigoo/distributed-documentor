...
public class VirtualTOCNode extends TOCNode {

    private static final Logger log = LoggerFactory.getLogger(VirtualTOCNode.class.getName());
    ...
    public ExportableNode getRealNode(File repositoryRoot, DocumentorPreferences prefs) {
        if (virtualHierarchyBuilder == null) {
            ...
            try {
                VirtualHierarchyBuilder builder = (VirtualHierarchyBuilder) ConstructorUtils.invokeConstructor(virtualHierarchyBuilder, new File(repositoryRoot, sourcePath), getTitle(), "MediaWiki");
